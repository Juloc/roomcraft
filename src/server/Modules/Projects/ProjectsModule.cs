...
public static class ProjectsModule
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            // A concurrent create can pass the existence check before either
            // request commits. Treat the database uniqueness constraint as the
            // authoritative race arbiter instead of leaking a 500.
            return Results.Conflict(new { error = "Project already exists." });
        }
    ...
    private static ProjectDocumentMetadata ReadMetadata(JsonElement document)
    {
        ...
        if (!document.TryGetProperty("schemaVersion", out var schemaVersionElement) ||
            ...
            schemaVersion is < 1 or > 6)
        {
            throw new ArgumentException("Project schemaVersion must be between 1 and 6.");
        }
