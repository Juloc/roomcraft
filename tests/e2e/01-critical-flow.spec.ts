...
test("builds, persists, reloads and exports a planned room", async ({ page }) => {
  ...
  // Wall drawing is a continuous tool, but right-click must always end it and
  // return to Select without adding another segment.
  await plan.click({ button: "right", position: { x: 60, y: 60 } });
  await expect(desktopTool(page, "Select and pan")).toHaveClass(/rc-button--primary/);
  await expect(statValue(page, "Walls")).toHaveText("4");
  ...
  await desktopTool(page, "Place door").click();
  await firstWall.click({ force: true });
  ...
  let menu = await openDesktopProjectMenu(page);
  const svgDownload = page.waitForEvent("download");
  await menu.getByRole("button", { name: "Export SVG", exact: true }).click();
  expect((await svgDownload).suggestedFilename()).toMatch(/\.svg$/);

  menu = await openDesktopProjectMenu(page);
  const glbDownload = page.waitForEvent("download");
  await menu.getByRole("button", { name: "Export GLB", exact: true }).click();
